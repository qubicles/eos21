        ...
        return eos.contract(eosioTokenAddress)
            .then(eosioToken => {
                createWormHole({
                    blackHole,
                    onData: event => {
                        const { amount, note, sender } = event.returnValues;
                        const amountFloat = (amount/10**decimals).toFixed(4);
                        const amountWithSymbol = amountFloat + " " + symbol;
                        console.log(new Date(), " (EVENT) amount=" + amountWithSymbol + ", to=" + note);
                        
                        eosioToken.issue(note, amountWithSymbol, "You've just been teleported from Ethereum - welcome to EOS.IO via Telos!")
                            .then( tx => {
                                eosioToken.addteleport(note, sender, amountWithSymbol).catch(console.error);
                            }).catch(console.error);
                    }
                });
